function startDetection() {
    ...
    fetch('/reconocimiento/detect_cameras/')
      ...
      .then(data => {
        ...
        // Si data.cameras no existe o no es array, detente aquí:
        ...
        data.cameras.forEach(camera => {
          ...
          if (camera.registered) {
            buttonHTML = `
              <button
                ...
                onclick="setDefaultCamera(${camera.id})">
            ...
            buttonHTML = `
              <button
                ...
                onclick="registerAndSetDefaultCamera(
                  '${camera.mac}',
                  '${camera.ip}',
                  '${camera.name}',
                  '${camera.location}'
          ...
          // Montamos todo el HTML dentro de un template literal
  ...
  function setDefaultCamera(cameraId) {
    fetch(`/set_default_camera/${cameraId}/`, {
      ...
      .catch(err => console.error('Error al establecer cámara:', err));
  }
  ...
  function registerAndSetDefaultCamera(mac, ip, name, location) {
    fetch(`/register_and_set_default_camera/`, {
      ...
      .catch(err => console.error('Error al registrar y establecer cámara:', err));
  }
function setDefaultCamera(cameraId) {
    fetch(`/set_default_camera/${cameraId}/`, {
        method: 'POST',
